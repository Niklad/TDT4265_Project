...
def pre_process_images(X: np.ndarray):
    ...
    X_proc = np.hstack((X_proc, np.ones((X_proc.shape[0],1))))
    # X_proc = np.hstack((X_proc, [1]))
    # np.hastack((a, b))
    #X_proc = np.append(X_proc, np.ones(X.shape[0]))
    # X_proc = np.insert(X_proc, X_proc.shape[1], 1, axis=1)
    # np.hstack([X, np.ones(X.shape[0])])
...
def cross_entropy_loss(targets: np.ndarray, outputs: np.ndarray) -> float:
    ...
    Cn = -(y * np.log(yhat) + (1 - y) * np.log(1 - yhat))
...
class BinaryModel:
    ...
    def backward(self, X: np.ndarray, outputs: np.ndarray, targets: np.ndarray) -> None:
        ...
        # Names to match variables in assignment
        yhat = outputs
        y = targets
